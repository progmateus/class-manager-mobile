...
export function StudentsClassList() {
  ...
  return (
    <View flex={1}>
      <PageHeader title="Alunos" rightIcon={tenantId ? Plus : null} rightAction={handleClickPlus} />
      <Viewcontainer>
              ...
              <VStack space={8}>
                {
                  students && students.length ? (
                    students.map((student: IUserDTO) => {
                      return (
                        <GenericItem.Root key={student.id}>
                          <GenericItem.Icon icon={SimCard} />
                          <GenericItem.Content title={student.name ?? ""} caption="" />
                        </GenericItem.Root>
                      )
                    })
                  )
